...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        `https://project-2-api.herokuapp.com/videos/?api_key=6eb81f40-ff68-4c0b-aa13-b2b7e9a62bf1`
      )
      .then((response) => {
        firstVideoId = response.data[0].id;
        const filteredVideosArray = response.data.filter(
          (video) => firstVideoId !== video.id
        );
        this.setState({
        ...
        axios
          .get(
            `https://project-2-api.herokuapp.com/videos/${firstVideoId}?api_key=6eb81f40-ff68-4c0b-aa13-b2b7e9a62bf1`
          )
          .then((responseTwo) => {
            this.setState({
          ...
          .catch((error) => {
            console.error(error.responseTwo);
          });
      })
      .catch((error) => {
        console.error(error.responseTwo);
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.state.details.id !== this.props.match.params.id) {
      ...
      //       axios
      //         .get(
      //           `https://project-2-api.herokuapp.com/videos/${VideoId}}/?api_key=6eb81f40-ff68-4c0b-aa13-b2b7e9a62bf1`
      //         )
      //         .then((response) => {
      //           this.setState({
  ...
  render() {
    ...
    //console.log("(this.state.loade", this.state.loaded);
